...
public abstract class CharacterSummoner extends GameObject implements IHandler {
    ...
    private static double IMAGE_SIZE = 1.0 / 16;
    private static double SPACING = 1.0 / 32;
    ...
    public void handle(AEvent pEvt, GameObject pSelf) {
        if (pEvt.getType() == EventType.UPDATE) {
            ...
            Transform cooldownTrans = new Transform(baseX, baseY + (IMAGE_SIZE - height - 1.0 / Main.HEIGHT) / 2.0, IMAGE_SIZE, height);
